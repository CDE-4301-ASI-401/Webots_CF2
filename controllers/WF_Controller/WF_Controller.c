...
#define LEFT_WALL_DISTANCE 0.4
#define RIGHT_WALL_DISTANCE 0.8
...
static float direction_arr[5] = { +1, -1, -1, -1, -1 };
...
int main(int argc, char **argv) {
  ...
  // float desired_angle = degrees_to_radians(heading[robot_id]);
  ...
  wall_follower_init(wall_distance, MAX_SPEED, 1);  // Set up wall following parameters
  ...
  while (wb_robot_step(timestep) != -1) {
    ...
    if (direction == -1){
      state_wf = wall_follower(&vel_x, &vel_y, &vel_w, range_front_value, range_left_value, heading, direction);
    }
    else {
      state_wf = wall_follower(&vel_x, &vel_y, &vel_w, range_front_value, range_right_value, heading, direction);
    }
    ...
    ///////////////////////////////////////////////////
    printf("Velocity yaw rate: %f\n", vel_w);  // Correct printf usage
    // printf("State %d\n", state);  // Correct printf usage
    if (height_desired-actual_state.altitude > 0.02){
      ...
      desired_state.vx = vel_x;
      desired_state.vy =  vel_y;
      desired_state.yaw_rate = direction * vel_w;
    }
