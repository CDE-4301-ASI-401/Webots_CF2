...
#define LEFT_WALL_DISTANCE 0.8
#define RIGHT_WALL_DISTANCE 1.5
...
int main(int argc, char **argv) {
  ...
  init_wall_follower_and_avoid_controller(wall_distance, MAX_SPEED, direction);  // Set up wall following parameters
  ...
  while (wb_robot_step(timestep) != -1) {
    ...
    int state = state = wall_follower_and_avoid_controller(&vel_x, &vel_y, &vel_w, range_front_value, range_left_value, range_right_value, heading,0.0);
    ...
    ///turn to find wall (does not have vel_w)
      ...
      desired_state.vx = vel_x;
      desired_state.vy = vel_y;
      desired_state.yaw_rate = MAX_SPEED;   
    }   
    else{
      printf("Loop 3\n");
      desired_state.vx = vel_x;
